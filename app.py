...
st.markdown(
    ...
    Gemini API Keyはユーザーが独自に取得して入力する必要あり：当ウェブアプリではGemini API を利用しており、Gemini API Keyはユーザーが独自に取得して入力する必要があります。その方法は下記の有料部分の中で示しています。ユーザーのもつGemini APIの使用制限によって利用回数には一定期間あたりの限度があります。著者はこれまで無料枠の範囲内で済んでいるものの、使用状況には御本人の責任で十分注意してください。<br>
    生成内容の正確性について：本ウェブアプリが生成する記事内容の正確性・最新性・完全性について保証しません。事実確認や情報の裏付けは、必ずユーザーご自身で行ってください。<br>
    法令遵守の義務：生成記事の利用にあたっては、著作権法・個人情報保護法・名誉毀損など関連法令を遵守する必要があります。違法または不適切な利用は固く禁止します。<br>
    第三者権利の侵害禁止：他者の権利（知的財産権・肖像権など）を侵害しないよう十分ご注意ください。生成結果をそのまま公開する場合、必要に応じて権利者からの許可を取得してください。<br>
    サービス仕様変更・終了の可能性：本ウェブアプリは予告なく仕様変更・機能追加・終了する場合があります。またStreamlit CloudやGithubが予期せぬ理由でサービス停止した場合には、このウェブアプリは使えなくなります。<br>
    利用環境の責任：APIキー管理やPC・ネットワーク環境のセキュリティは、ユーザーの責任で確保してください。
    </p>
    """,
    unsafe_allow_html=True
)
